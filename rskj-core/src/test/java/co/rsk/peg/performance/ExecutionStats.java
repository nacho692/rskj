...
public class ExecutionStats {
    ...
    public String getPrintable() {
        return String.format(
                "%-45s\tgas: %s\t\tcpu(us): %d\t\treal(us): %d\t\twrt(slots): %d\t\tclr(slots): %d\t\tgasForData: %d\t\texecTime/gasForData(ns/gas): %d",
                name,
                ...
                gasForData.getMean(),
                executionTimes.getMean()/gasForData.getMean()
        );
    }
}
